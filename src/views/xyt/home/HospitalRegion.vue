    ...
    <div class="region">
        <div class="content">
            ...
            <ul class="hospital_region">
                ...
                <li v-for="item in hospitalRegionList" :key="item.id" :class="{active:activedRegionFlag==item.id }"  @click="changeRegion(item.id)">{{ item.district_name }}</li>
            </ul>
        </div>
    </div>
</template>
...
const getHospitalRegionData = async ()=>{
    ...
    console.log(hospitalRegionList.value);
}

const changeRegion = (region:number)=>{
    activedRegionFlag.value = region
}
</script>
