<template>
  <div class="register">
    ...
    <div class="top">
      <div class="title">
        <span>{{ hosDetail.hospitalDetail?.full_name }}</span>
      </div>
      <div class="grade">
        <svg
          t="1745935908882"
        ...
        <span>{{ hosDetail.hospitalDetail?.grade_name }}</span>
      </div>
    </div>
    <!-- 内容 -->
    <div class="content">
      ...
      <div class="right">
        <div>挂号规则</div>
        <div class="time">预约周期: 10天 放号时间: 08:00 停挂时间: 12:30</div>
        <div class="canceltime">退号时间: 就诊前一天15:30前取消</div>
        <div class="address">具体位置: {{ hosDetail.hospitalDetail?.address }}</div>
      </div>
    </div>
    <!-- 医院科室数据 -->
    <div class="department" v-loding="loading">
      ...
      <div class="leftNav">
        <ul>
          <li
            ...
            v-for="(dpt, index) in hosDetail.departmentList"
            :key="dpt.uid"
          >
            {{ dpt.name }}
          </li>
      ...
      <!-- 二级科室列表 -->
      <div class="rightNav">
        <div class="showDepartment" v-for="dpt in hosDetail.departmentList" :key="dpt.uid">
