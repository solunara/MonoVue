<template>
  <div class="container">
    ...
    <!-- 确认挂号按钮 -->
    <div class="btn">
      <el-button type="primary" size="default" :disabled="currentIndex == -1 ? true : false" @click="submitOrder">
        确认挂号
      </el-button>
    </div>
  </div>
...
const getPatientList = async () => {
  let result: ResponsePatientsData = await getPatientData()
  if (result.code == 200) {
...
const submitOrder = async () => {
  //就诊人的ID
  let patientId: string = patientList.value[currentIndex.value].id as string
