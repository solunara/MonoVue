  ...
  <div class="wrap">
    ...
    <div class="center">
      <div class="container">
        <div
          class="item"
          v-for="ele in deptSchedulerData"
          :key="ele.date"
          :class="{ active: (hasReached3PM() && isToday(ele.date)) || ele.remain == 0 }"
          @click="changeTime(ele.date, ele.docScheduler)"
        >
          <div class="header">{{ ele.date }} {{ formatWeek(ele.weekday) }}</div>
          <div class="bottom">
            <div class="itemstatus" v-if="hasReached3PM() && isToday(ele.date)">停止挂号</div>
            <div class="itemstatus" v-else-if="ele.remain > 0">剩余({{ ele.remain }})</div>
            <div class="itemstatus" v-else>已约满</div>
          </div>
        </div>
      </div>
      <!-- 分页器 -->
    ...
    <div class="selectdata">{{ selectDate }} 医生排班信息：</div>
    <!-- 展示医生 -->
    <div class="fotter">
      <!-- 展示即将放号的时间 -->
          ...
          <div class="doc_info" v-for="doctor in moringArr" :key="doctor.docId" v-if="isSchedulerMorning">
            ...
            <div class="right">
              ...
              <el-button
                type="primary"
                size="default"
                :disabled="hasReached3PM() && isToday(doctor.workDay)"
                @click="goStep2(doctor)"
                >{{ doctor.maxPatients - doctor.registered }}</el-button
              >
            </div>
          </div>
          <div v-else>
...
const getHospitalRegistrationList = async () => {
  let result: ScheduleInfo = await getHospitalScheduler(
    ($route.query.hosId ?? "") as string,
    ($route.query.deptId ?? "") as string,
    pageNo.value,
    pageSize.value
  )
  if (result.code === 200) {
